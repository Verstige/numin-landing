-- Nexus AI Business Suite Migration
-- This script adds the Nexus AI tables to your existing Supabase database
...
-- RAG Documents table (simplified without vector for now)
CREATE TABLE IF NOT EXISTS rag_documents (
  ...
  embedding JSONB DEFAULT '[]', -- Store as JSON array for now
...
-- Agent Vector Memories table
CREATE TABLE IF NOT EXISTS agent_vector_memories (
  id TEXT PRIMARY KEY,
  agent_id TEXT REFERENCES ai_agents(id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  embedding JSONB DEFAULT '[]', -- Store as JSON array for now
...
CREATE INDEX IF NOT EXISTS idx_api_connectors_team_id ON api_connectors(team_id);
CREATE INDEX IF NOT EXISTS idx_api_connectors_type ON api_connectors(type);
...
CREATE INDEX IF NOT EXISTS idx_business_insights_team_id ON business_insights(team_id);
...
ALTER TABLE agent_vector_memories ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Team access for workflows" ON workflows
  FOR ALL USING (
    team_id IN (
      SELECT team_id FROM team_members 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Team access for api_connectors" ON api_connectors
  FOR ALL USING (
    team_id IN (
      SELECT team_id FROM team_members 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Team access for rag_documents" ON rag_documents
  FOR ALL USING (
    team_id IN (
      SELECT team_id FROM team_members 
      WHERE user_id = auth.uid()
